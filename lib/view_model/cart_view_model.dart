...
// Add product to database method
  addProduct(CartModel cartModel) async {
    ...
    _totalPrice += (double.parse(cartModel.productPrice) * cartModel.productQuantity);
    Get.snackbar(
      'Successful',
...
// Calculate total price method
  getTotalPrice() {
    for (int i = 0; i < _cartModel.length; i++) {
      _totalPrice += (double.parse(_cartModel[i].productPrice) * _cartModel[i].productQuantity);
      update();
    }
  }
